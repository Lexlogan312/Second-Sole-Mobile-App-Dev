  ...
  if (!isSetup) {
    return (
      <div className={`min-h-screen bg-[${THEME.bg}] flex flex-col items-center justify-center p-6 relative overflow-hidden`}>
        <div className={`absolute top-0 left-0 w-full h-1/2 bg-gradient-to-b from-[${THEME.surface}] to-transparent opacity-50 -z-10`} />

        <div className="w-full max-w-sm">
          <h1 className="text-4xl font-bold text-white mb-2">Second Sole<br /><span className="gradient-text">Medina</span></h1>
          <p className={`text-[${THEME.muted}] mb-8`}>Precision fit. Local expertise. Digital speed.</p>
          ...
          <p className={`text-xs text-center text-[${THEME.muted}]/50 mt-8`}>
  ...
  return (
    <div className={`min-h-screen bg-[${THEME.bg}] flex flex-col items-center justify-center p-6 text-center`}>
      <div className="mb-12 relative">
        <div className={`w-32 h-32 rounded-full border-2 flex items-center justify-center transition-all duration-700 ${isScanning ? `border-[${THEME.accent}] shadow-[0_0_50px_rgba(228,57,40,0.5)]` : `border-[${THEME.surface}]`}`}>
          <ScanFace size={48} className={isScanning ? `text-[${THEME.accent}] animate-pulse` : 'text-white'} />
        </div>
        {isScanning && (
      ...
      <p className={`text-[${THEME.muted}] mb-12`}>System Locked. FaceID Required.</p>

      <Button size="lg" className="w-48" onClick={handleUnlock}>
        {isScanning ? 'Verifying...' : 'Unlock'}
      </Button>

      <div className={`flex items-center gap-2 mt-8 text-xs text-[${THEME.muted}]`}>
        <Lock size={12} />
